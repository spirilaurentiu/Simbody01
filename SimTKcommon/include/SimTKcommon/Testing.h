...
class Test {
    ...
    template <class P>
    static bool
    numericallyEqual(const std::complex<P>& v1, const std::complex<P>& v2, int n, double tol = defTol<P>()) {
        return numericallyEqual(v1.real(), v2.real(), n, tol)
               && numericallyEqual(v1.imag(), v2.imag(), n, tol);
    }
    template <class P>
    static bool
    numericallyEqual(const conjugate<P>& v1, const conjugate<P>& v2, int n, double tol = defTol<P>()) {
        return numericallyEqual(v1.real(), v2.real(), n, tol)
               && numericallyEqual(v1.imag(), v2.imag(), n, tol);
    }
    template <class P>
    static bool
    numericallyEqual(const std::complex<P>& v1, const conjugate<P>& v2, int n, double tol = defTol<P>()) {
        return numericallyEqual(v1.real(), v2.real(), n, tol)
               && numericallyEqual(v1.imag(), v2.imag(), n, tol);
    }
    template <class P>
    static bool
    numericallyEqual(const conjugate<P>& v1, const std::complex<P>& v2, int n, double tol = defTol<P>()) {
        return numericallyEqual(v1.real(), v2.real(), n, tol)
               && numericallyEqual(v1.imag(), v2.imag(), n, tol);
    }
    template <class P>
    static bool
    numericallyEqual(const negator<P>& v1, const negator<P>& v2, int n, double tol = defTol<P>()) {
    ...
    static bool numericallyEqual(const Vec<M, E1, S1>& v1,
        ...
        for (int i = 0; i < M; ++i) {
            if (!numericallyEqual(v1[i], v2[i], n, tol)) {
                return false;
            }
        }
        return true;
    ...
    static bool numericallyEqual(const Row<N, E1, S1>& v1,
                                 ...
                                 double tol = (defTol2<E1, E2>())) {
        for (int j = 0; j < N; ++j) {
            if (!numericallyEqual(v1[j], v2[j], n, tol)) {
                return false;
            }
        }
        return true;
    ...
    template <class E1, class E2>
    static bool
    numericallyEqual(const Vector_<E1>& v1, const Vector_<E2>& v2, int n, double tol = (defTol2<E1, E2>())) {
    ...
    static bool numericallyEqual(const MatrixView_<E1>& v1,
                                 ...
                                 int n,
                                 double tol = (defTol2<E1, E2>())) {
        if (v1.nrow() != v2.nrow() || v1.ncol() != v2.ncol()) {
            return false;
        }
    ...
    template <class E1, class E2>
    static bool
    numericallyEqual(const Matrix_<E1>& m1, const Matrix_<E2>& m2, int n, double tol = (defTol2<E1, E2>())) {
    ...
    template <class E1, class E2>
    static bool numericallyEqual(const MatrixView_<E1>& m1,
                                 const Matrix_<E2>& m2,
                                 int n,
                                 double tol = (defTol2<E1, E2>())) {
        return numericallyEqual((const MatrixView_<E1>&)m1, (const MatrixView_<E2>&)m2, n, tol);
    ...
    template <class P>
    static bool
    numericallyEqual(const Rotation_<P>& R1, const Rotation_<P>& R2, int n, double tol = defTol<P>()) {
        return R1.isSameRotationToWithinAngle(R2, (Real)(n * tol));
    }

    template <class P>
    static bool
    numericallyEqual(const Transform_<P>& T1, const Transform_<P>& T2, int n, double tol = defTol<P>()) {
        return numericallyEqual(T1.R(), T2.R(), n, tol) && numericallyEqual(T1.p(), T2.p(), n, tol);
    }

    template <class P>
    static bool
    numericallyEqual(const UnitInertia_<P>& G1, const UnitInertia_<P>& G2, int n, double tol = defTol<P>()) {
        return numericallyEqual(G1.asSymMat33(), G2.asSymMat33(), n, tol);
    }

    template <class P>
    static bool
    numericallyEqual(const Inertia_<P>& I1, const Inertia_<P>& I2, int n, double tol = defTol<P>()) {
    ...
    static SpatialMat randSpatialMat() {
        return SpatialMat(randMat33(), randMat33(), randMat33(), randMat33());
    }
    ...
    --> */
    static void
    PrintVec3(SimTK::Vec3 V, int decimal_places, std::string header = "unknown", std::string rowPrefix = "") {
    ...
    /*! <-- Print Transform
    --> */
    static void PrintTransform(SimTK::Transform T,
                               int decimal_places,
...
#define SimTK_END_TEST()                                                        \
        ...
        std::cerr << "Test failed due to exception: " << e.what() << std::endl; \
        return 1;                                                               \
    }                                                                           \
    catch (...) {                                                               \
...
#define SimTK_SUBTEST(testFunction)              \
    do {                                         \
        SimTK::Test::Subtest sub(#testFunction); \
        (testFunction)();                        \
    } while (false)
...
#define SimTK_TEST_EQ_SIZE(v1, v2, n)                                                                  \
    {                                                                                                  \
        SimTK_ASSERT1_ALWAYS(                                                                          \
            SimTK::Test::numericallyEqual((v1), (v2), (n)),                                            \
            "Test values should have been numerically equivalent at size=%d times default tolerance.", \
            (n));                                                                                      \
    }
...
#define SimTK_TEST_EQ_TOL(v1, v2, tol)                                                               \
    {                                                                                                \
        SimTK_ASSERT1_ALWAYS(SimTK::Test::numericallyEqual((v1), (v2), 1, (tol)),                    \
                             "Test values should have been numerically equivalent at tolerance=%g.", \
                             (tol));                                                                 \
    }
...
#define SimTK_TEST_NOTEQ(v1, v2)                                                                \
    {                                                                                           \
        SimTK_ASSERT_ALWAYS(                                                                    \
            !SimTK::Test::numericallyEqual((v1), (v2), 1),                                      \
...
#define SimTK_TEST_NOTEQ_SIZE(v1, v2, n)                                                                   \
    {                                                                                                      \
        SimTK_ASSERT1_ALWAYS(                                                                              \
            !SimTK::Test::numericallyEqual((v1), (v2), (n)),                                               \
            "Test values should NOT have been numerically equivalent at size=%d times default tolerance.", \
            (n));                                                                                          \
    }
...
#define SimTK_TEST_NOTEQ_TOL(v1, v2, tol)                                                                \
    {                                                                                                    \
        SimTK_ASSERT1_ALWAYS(!SimTK::Test::numericallyEqual((v1), (v2), 1, (tol)),                       \
                             "Test values should NOT have been numerically equivalent at tolerance=%g.", \
                             (tol));                                                                     \
    }
...
#    define SimTK_TEST_MUST_THROW(stmt)                                                                  \
        do {                                                                                             \
            ...
            if (threw == 2)                                                                              \
                SimTK_TEST_FAILED1(                                                                      \
                    "Expected statement\n%s\n  to throw an std::exception but it threw something else.", \
                    #stmt);                                                                              \
        } while (false)
        ...
        do {                                                                                               \
            ...
            if (threw == 0)                                                                                \
                SimTK_TEST_FAILED1(                                                                        \
                    ...
                    #stmt);                                                                                \
            if (threw == 2)                                                                                \
                SimTK_TEST_FAILED2("Expected statement\n----\n%s\n----\n  to throw exception type %s but " \
