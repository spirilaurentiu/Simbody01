...
template <class S>
void MatrixHelper<S>::deleteRepIfOwner() {
...
template <class S>
void MatrixHelper<S>::replaceRep(MatrixHelperRep<S>* hrep) {
    deleteRepIfOwner();
    rep = hrep;
...
// of this rep.
template <class S>
MatrixHelper<S>::MatrixHelper(MatrixHelperRep<S>* hrep)
    : rep(hrep) {
    if (rep) {
        rep->setMyHandle(*this);
    }
...
template <class S>
const MatrixCommitment& MatrixHelper<S>::getCharacterCommitment() const {
    return getRep().getCharacterCommitment();
}

template <class S>
const MatrixCharacter& MatrixHelper<S>::getMatrixCharacter() const {
...
template <class S>
MatrixHelper<S>::MatrixHelper(int esz, int cppEsz, const MatrixCommitment& mc)
    : rep(0) {
...
template <class S>
MatrixHelper<S>::MatrixHelper(int esz, int cppEsz)
    : rep(0) {
...
template <class S>
MatrixHelper<S>::MatrixHelper(int esz, int cppEsz, const MatrixCommitment& commitment, int m, int n)
    : rep(0) {
    SimTK_ERRCHK2(commitment.isSizeOK(m, n),
                  "MatrixHelper::ctor()",
...
template <class S>
MatrixHelper<S>::MatrixHelper(int esz,
                              int cppEsz,
                              const MatrixCommitment& commitment,
                              const MatrixCharacter& actual,
                              int spacing,
                              const S* data)
    : rep(0) {
    SimTK_ERRCHK(commitment.isSatisfiedBy(actual),
                 "MatrixHelper::ctor(external data)",
                 "The supplied actual matrix character for the external data did not "
                 "satisfy the specified handle character commitment.");

    rep = MatrixHelperRep<S>::createExternalMatrixHelperRep(esz,
                                                            cppEsz,
                                                            actual,
                                                            spacing,
                                                            const_cast<S*>(data),
...
MatrixHelper<S>::MatrixHelper(int esz,
    ...
    rep = MatrixHelperRep<S>::createExternalMatrixHelperRep(esz, cppEsz, actual, spacing, data, true);
    assert(rep);
    rep->setMyHandle(*this);
...
void MatrixHelper<S>::clear() {
    ...
    MatrixHelperRep<S>* newRep = MatrixHelperRep<S>::createOwnerMatrixHelperRep(esz, cppEsz, actual);
    assert(newRep);
...
template <class S>
bool MatrixHelper<S>::isClear() const {
    return getRep().isClear();
}


template <class S>
void MatrixHelper<S>::commitTo(const MatrixCommitment& mc) {
    SimTK_ERRCHK_ALWAYS(isClear(),
                        "MatrixHelper::commitTo()",
...
template <class S>
MatrixHelper<S>::MatrixHelper(const MatrixCommitment& mc, const MatrixHelper& h, const ShallowCopy&)
    : rep(0) {
    SimTK_ERRCHK(mc.isSatisfiedBy(h.getMatrixCharacter()),
                 "MatrixHelper::ctor(const,shallow)",
...
template <class S>
MatrixHelper<S>::MatrixHelper(const MatrixCommitment& mc, MatrixHelper& h, const ShallowCopy&)
    : rep(0) {
    SimTK_ERRCHK(mc.isSatisfiedBy(h.getMatrixCharacter()),
                 "MatrixHelper::ctor(writable,shallow)",
                 "The actual matrix character of the source did not "
                 "satisfy the specified handle character commitment.");
...
template <class S>
MatrixHelper<S>::MatrixHelper(const MatrixCommitment& mc, const MatrixHelper& h, const DeepCopy&)
    : rep(0) {
    SimTK_ERRCHK(mc.isSatisfiedBy(h.getMatrixCharacter()),
                 "MatrixHelper::ctor(deep)",
                 "The actual matrix character of the source did not "
                 "satisfy the specified handle character commitment.");
...
// vice versa).
template <class S>
MatrixHelper<S>::MatrixHelper(const MatrixCommitment& mc,
                              const MatrixHelper<typename CNT<S>::TNeg>& h,
                              const DeepCopy&)
    : rep(0) {
    SimTK_ERRCHK(mc.isSatisfiedBy(h.getMatrixCharacter()),
                 "MatrixHelper::ctor(negated,deep)",
                 "The actual matrix character of the source did not "
                 "satisfy the specified handle character commitment.");
...
template <class S>
MatrixHelper<S>::MatrixHelper(const MatrixCommitment& mc, const MatrixHelper& h, int i, int j, int m, int n)
    : rep(0) {
    if (n == 1) {
...
template <class S>
MatrixHelper<S>::MatrixHelper(const MatrixCommitment& mc, MatrixHelper& h, int i, int j, int m, int n)
    : rep(0) {
    if (n == 1) {
    ...
    SimTK_ERRCHK(mc.isSatisfiedBy(rep->getMatrixCharacter()),
                 "MatrixHelper::ctor(writable,block)",
                 "The actual matrix character of the source block did not "
                 "satisfy the specified handle character commitment.");
...
template <class S>
MatrixHelper<S>::MatrixHelper(const MatrixCommitment& mc,
                              const MatrixHelper<typename CNT<S>::THerm>& h,
                              const TransposeView&)
    : rep(0) {
    rep = h.rep->createHermitianView(false);

    SimTK_ERRCHK(mc.isSatisfiedBy(rep->getMatrixCharacter()),
                 "MatrixHelper::ctor(const,transpose)",
...
template <class S>
MatrixHelper<S>::MatrixHelper(const MatrixCommitment& mc,
                              MatrixHelper<typename CNT<S>::THerm>& h,
                              const TransposeView&)
    : rep(0) {
    rep = h.rep->createHermitianView(true);

    SimTK_ERRCHK(mc.isSatisfiedBy(rep->getMatrixCharacter()),
                 "MatrixHelper::ctor(writable,transpose)",
                 "The actual matrix character of the transposed source did not "
                 "satisfy the specified handle character commitment.");
...
template <class S>
MatrixHelper<S>::MatrixHelper(const MatrixCommitment& mc, const MatrixHelper& h, const DiagonalView&)
    : rep(0) {
    rep = h.rep->createDiagonalView(false);

    SimTK_ERRCHK(mc.isSatisfiedBy(rep->getMatrixCharacter()),
                 "MatrixHelper::ctor(const,diagonal)",
...
template <class S>
MatrixHelper<S>::MatrixHelper(const MatrixCommitment& mc, MatrixHelper& h, const DiagonalView&)
    : rep(0) {
...
template <class S>
MatrixHelper<S>::MatrixHelper(const MatrixCommitment& mc, const MatrixHelper& h, int n, const int* ix)
    : rep(0) {
    const VectorHelper<S>& vh = SimTK_DYNAMIC_CAST_DEBUG<const VectorHelper<S>&>(*h.rep);
    rep = new IndexedVectorHelper<S>(vh.getEltSize(),
                                     vh.getCppEltSize(),
                                     n,
                                     vh.preferRowOrder_(),
                                     ix,
...
template <class S>
MatrixHelper<S>::MatrixHelper(const MatrixCommitment& mc, MatrixHelper& h, int n, const int* ix)
    : rep(0) {
...
template <class S>
MatrixHelper<S>& MatrixHelper<S>::copyAssign(const MatrixHelper& h) {
    if (rep->isOwner()) {
...
template <class S>
MatrixHelper<S>& MatrixHelper<S>::negatedCopyAssign(const MatrixHelper<typename CNT<S>::TNeg>& h) {
    if (rep->isOwner()) {
        rep->resize(h.nrow(), h.ncol(), false);
    } else {
        // OK if the sizes match.
...
template <class S>
MatrixHelper<S>& MatrixHelper<S>::readOnlyViewAssign(const MatrixHelper& h) {
...
// Create a (possibly) writable view of existing data.
template <class S>
MatrixHelper<S>& MatrixHelper<S>::writableViewAssign(MatrixHelper& h) {
    SimTK_ERRCHK(getCharacterCommitment().isSatisfiedBy(h.getMatrixCharacter()),
...
template <class S>
void MatrixHelper<S>::scaleBy(const typename CNT<S>::StdNumber& s) {
    rep->scaleBy(s);
}
template <class S>
void MatrixHelper<S>::addIn(const MatrixHelper& h) {
    rep->addIn(h);
}
template <class S>
void MatrixHelper<S>::addIn(const MatrixHelper<typename CNT<S>::TNeg>& h) {
    subIn(reinterpret_cast<const MatrixHelper<S>&>(h));
}
template <class S>
void MatrixHelper<S>::subIn(const MatrixHelper& h) {
    rep->subIn(h);
}
template <class S>
void MatrixHelper<S>::subIn(const MatrixHelper<typename CNT<S>::TNeg>& h) {
    addIn(reinterpret_cast<const MatrixHelper<S>&>(h));
}
template <class S>
void MatrixHelper<S>::fillWith(const S* eltp) {
    rep->fillWith(eltp);
}
template <class S>
void MatrixHelper<S>::copyInByRowsFromCpp(const S* elts) {
    rep->copyInByRowsFromCpp(elts);
}

template <class S>
void MatrixHelper<S>::invertInPlace() {
    rep->invertInPlace();
}

template <class S>
void MatrixHelper<S>::dump(const char* msg) const {
    rep->dump(msg);
}

template <class S>
const S* MatrixHelper<S>::getElt(int i, int j) const {
    return rep->getElt(i, j);
}
template <class S>
S* MatrixHelper<S>::updElt(int i, int j) {
    return rep->updElt(i, j);
}
template <class S>
void MatrixHelper<S>::getAnyElt(int i, int j, S* value) const {
    return rep->getAnyElt(i, j, value);
}

template <class S>
const S* MatrixHelper<S>::getElt(int i) const {
    return rep->getElt(i);
}
template <class S>
S* MatrixHelper<S>::updElt(int i) {
    return rep->updElt(i);
}
template <class S>
void MatrixHelper<S>::getAnyElt(int i, S* value) const {
    return rep->getAnyElt(i, value);
}

template <class S>
int MatrixHelper<S>::nrow() const {
    return rep->nrow();
}
template <class S>
int MatrixHelper<S>::ncol() const {
    return rep->ncol();
}
template <class S>
ptrdiff_t MatrixHelper<S>::nelt() const {
    return rep->nelt();
}
template <class S>
int MatrixHelper<S>::length() const {
    return rep->length();
}

template <class S>
void MatrixHelper<S>::resize(int m, int n) {
    rep->resize(m, n, false);
}
template <class S>
void MatrixHelper<S>::resizeKeep(int m, int n) {
    rep->resize(m, n, true);
}

template <class S>
void MatrixHelper<S>::lockShape() {
    rep->lockHandle();
}
template <class S>
void MatrixHelper<S>::unlockShape() {
    rep->unlockHandle();
}

template <class S>
void MatrixHelper<S>::sum(S* const answer) const {
    rep->sum(answer);
}
template <class S>
void MatrixHelper<S>::colSum(int j, S* const answer) const {
    rep->colSum(j, answer);
}
template <class S>
void MatrixHelper<S>::rowSum(int i, S* const answer) const {
    rep->rowSum(i, answer);
}
template <class S>
void MatrixHelper<S>::fillWithScalar(const typename CNT<S>::StdNumber& s) {
    rep->fillWithScalar(s);
}

template <class S>
bool MatrixHelper<S>::hasContiguousData() const {
    return rep->hasContiguousData();
}
template <class S>
ptrdiff_t MatrixHelper<S>::getContiguousDataLength() const {
    assert(hasContiguousData());
    return rep->nScalars();
}
template <class S>
const S* MatrixHelper<S>::getContiguousData() const {
    assert(hasContiguousData());
    return rep->m_data;
}
template <class S>
S* MatrixHelper<S>::updContiguousData() {
    assert(hasContiguousData());
    return rep->m_data;
}
template <class S>
void MatrixHelper<S>::replaceContiguousData(S* newData, ptrdiff_t length, bool takeOwnership) {
...
template <class S>
void MatrixHelper<S>::replaceContiguousData(const S* newData, ptrdiff_t length) {
    replaceContiguousData(const_cast<S*>(newData), length, false);
    rep->m_writable = false;
}
template <class S>
void MatrixHelper<S>::swapOwnedContiguousData(S* newData, ptrdiff_t length, S*& oldData) {
    assert(length == getContiguousDataLength());
    assert(rep->m_owner);
    oldData = rep->m_data;
    rep->m_data = newData;
...
MatrixHelperRep<S>::createOwnerMatrixHelperRep(int esz, int cppEsz, const MatrixCharacter& want) {
    SimTK_ASSERT2((esz == 1 && cppEsz == 1) || (esz > 1 && cppEsz >= esz),
                  "MatrixHelperRep::createOwnerMatrixHelperRep(): bad element size esz=%d, cppEsz=%d",
                  esz,
                  cppEsz);
    ...
    switch (structure.getStructure()) {
        ...
        case MatrixStructure::SkewHermitian: {
            ...
            if (/*storage.getPlacement() == MatrixStorage::Upper*/ true) {
                rep = (esz == 1 ? (TriInFullHelper<S>*)new TriInFullUpperHelper<
                                      S>(1, 1, nr, nc, triangular, hermitian, skew, rowOrder)
                                : (TriInFullHelper<S>*)new TriInFullUpperHelper<
                                      S>(esz, cppEsz, nr, nc, triangular, hermitian, skew, rowOrder));
            }
            // else
        ...
        case MatrixStructure::Matrix1d: {
            ...
            rep = (esz == 1
                       ? (FullVectorHelper<S>*)new ContiguousVectorScalarHelper<S>(length, rowOrder)
                       : (FullVectorHelper<S>*)new ContiguousVectorHelper<S>(esz, cppEsz, length, rowOrder));
            break;
        }

        default:
            SimTK_ERRCHK1(!"not implemented",
                          "MatrixHelperRep::createOwnerMatrixHelperRep()",
...
template <class S>
MatrixHelperRep<S>* MatrixHelperRep<S>::createExternalMatrixHelperRep(int esz,
                                                                      int cppEsz,
                                                                      const MatrixCharacter& actual,
                                                                      int spacing,
                                                                      S* data,
                                                                      bool canWrite) {
    SimTK_ASSERT2((esz == 1 && cppEsz == 1) || (esz > 1 && cppEsz >= esz),
                  "MatrixHelperRep::createExternalMatrixHelperRep(): bad element size esz=%d, cppEsz=%d",
                  esz,
                  cppEsz);
    ...
    switch (structure.getStructure()) {
        case MatrixStructure::Full:
            if (rowOrder) {
                rep = (esz == 1 ? (RegularFullHelper<S>*)new FullRowOrderScalarHelper<S>(nr,
                                                                                         nc,
                                                                                         spacing,
                                                                                         data,
                                                                                         canWrite)
                                : (RegularFullHelper<S>*)new FullRowOrderEltHelper<S>(esz,
                                                                                      cppEsz,
                                                                                      nr,
                                                                                      nc,
                                                                                      spacing,
                ...
                rep = (esz == 1 ? (RegularFullHelper<S>*)new FullColOrderScalarHelper<S>(nr,
                                                                                         nc,
                                                                                         spacing,
                                                                                         data,
                                                                                         canWrite)
                                : (RegularFullHelper<S>*)new FullColOrderEltHelper<S>(esz,
                                                                                      cppEsz,
                                                                                      nr,
                                                                                      nc,
                                                                                      spacing,
        ...
        case MatrixStructure::SkewHermitian: {
            ...
            if (/*storage.getPlacement() == MatrixStorage::Upper*/ true) {
                rep = (esz == 1 ? (TriInFullHelper<S>*)new TriInFullUpperHelper<S>(1,
                                                                                   1,
                                                                                   nr,
                                                                                   nc,
                                                                                   triangular,
                                                                                   ...
                                                                                   data,
                                                                                   canWrite)
                                : (TriInFullHelper<S>*)new TriInFullUpperHelper<S>(esz,
                                                                                   cppEsz,
                                                                                   nr,
                                                                                   nc,
                                                                                   triangular,
        ...
        case MatrixStructure::Matrix1d: {
            ...
            if (strideInElements > 1) {
                rep = (esz == 1 ? (FullVectorHelper<S>*)new StridedVectorScalarHelper<S>(length,
                                                                                         rowOrder,
                                                                                         strideInElements,
                                                                                         data,
                                                                                         canWrite)
                                : (FullVectorHelper<S>*)new StridedVectorHelper<S>(esz,
                                                                                   cppEsz,
                                                                                   length,
                                                                                   rowOrder,
                                                                                   strideInElements,
                ...
                rep = (esz == 1 ? (FullVectorHelper<S>*)new ContiguousVectorScalarHelper<S>(length,
                                                                                            rowOrder,
                                                                                            data,
                                                                                            canWrite)
                                : (FullVectorHelper<S>*)new ContiguousVectorHelper<S>(esz,
                                                                                      cppEsz,
                                                                                      length,
                                                                                      rowOrder,
                                                                                      data,
...
template <class S>
void MatrixHelperRep<S>::scaleBy(const typename CNT<S>::StdNumber& s) {
...
template <class S>
void MatrixHelperRep<S>::addIn(const MatrixHelper<S>& h) {
...
template <class S>
void MatrixHelperRep<S>::addIn(const MatrixHelper<typename CNT<S>::TNeg>& nh) {
    subIn(reinterpret_cast<const MatrixHelper<S>&>(nh));
}

template <class S>
void MatrixHelperRep<S>::subIn(const MatrixHelper<S>& h) {
    const MatrixHelperRep& hrep = h.getRep();
...
template <class S>
void MatrixHelperRep<S>::subIn(const MatrixHelper<typename CNT<S>::TNeg>& nh) {
    addIn(reinterpret_cast<const MatrixHelper<S>&>(nh));
}

template <class S>
void MatrixHelperRep<S>::fillWith(const S* eltp) {
...
template <class S>
void MatrixHelperRep<S>::copyInByRowsFromCpp(const S* elts) {
...
template <class S>
static void dumpElt(const S* p, int sz) {
    if (sz > 1) {
        std::cout << "{";
    }
    for (int k = 0; k < sz; ++k) {
...
template <class S>
void MatrixHelperRep<S>::dump(const char* msg) const {
    if (msg) {
        std::cout << std::string(msg) << std::endl;
    }
    std::cout << "Matrix " << nrow() << " X " << ncol() << " " << getEltSize()
              << "-scalar entries:" << std::endl;
    if (nrow() * ncol() == 0) {
...
template <class S>
VectorHelper<S>* RegularFullHelper<S>::createDiagonalView_() {
    VectorHelper<S>* p = 0;
    const int length = std::min(this->nrow(), this->ncol());
    S* data = length ? this->updElt_(0, 0) : 0;

    const int strideInScalars = length > 1 ? int(this->getElt_(1, 1) - this->getElt_(0, 0)) : this->m_eltSize;
    ...
    if (strideInElements == 1) {
        p = (this->m_eltSize == 1)
                ...
                : (VectorHelper<S>*)new ContiguousVectorHelper<S>(this->m_eltSize,
                                                                  this->m_cppEltSize,
                                                                  length,
                                                                  false,
                                                                  data,
    ...
    p = (this->m_eltSize == 1)
            ...
            : (VectorHelper<S>*)new StridedVectorHelper<S>(this->m_eltSize,
                                                           this->m_cppEltSize,
                                                           length,
                                                           false,
                                                           strideInElements,
...
template <class S>
VectorHelper<S>* RegularFullHelper<S>::createColumnView_(int j, int i, int m) {
    VectorHelper<S>* p = 0;
...
template <class S>
VectorHelper<S>* RegularFullHelper<S>::createRowView_(int i, int j, int n) {
    VectorHelper<S>* p = 0;
    S* data = n ? this->updElt_(i, j) : 0;

    const int strideInScalars = n > 1 ? int(this->getElt_(i, j + 1) - this->getElt_(i, j)) : this->m_eltSize;
    const int strideInElements = strideInScalars / this->m_eltSize;
...
template <class S>
VectorHelper<S>* TriInFullHelper<S>::createDiagonalView_() {
    SimTK_ERRCHK_ALWAYS(!hasKnownDiagonal(),
                        "TriInFullHelper::createDiagonalView_()",
