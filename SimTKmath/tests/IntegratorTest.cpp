...
class MyPendulumGuts : public System::Guts {
    ...
    /*virtual*/ MyPendulumGuts* cloneImpl() const override {
    ...
    void projectQImpl(State&,
                      Vector& qErrEst,
                      const ProjectOptions& options,
                      ProjectResults& results) const override;
    void projectUImpl(State&,
                      Vector& uErrEst,
                      const ProjectOptions& options,
                      ProjectResults& results) const override;
    ...
    int calcEventTriggerInfoImpl(const State& s, Array_<EventTriggerInfo>& eti) const override {
        eti.clear();
        eti.push_back(
    ...
    int calcTimeOfNextScheduledEventImpl(const State& s,
                                         Real& tNextEvent,
        ...
        // Careful ...
        if (tNextEvent < s.getTime() || (tNextEvent == s.getTime() && !includeCurrentTime)) {
    ...
    void handleEventsImpl(State& s,
        ...
        cout << "===> t=" << s.getTime() << ": HANDLING " << Event::getCauseName(cause) << " EVENT!!!"
             << endl;
        if (eventIds.size()) {
...
static void reportState(const char* msg, const System& sys, const Integrator& integ) {
    if (*msg) {
        printf("%s\n", msg);
    }
    ...
    sys.realize(s);
    printf(" -%6s- %9.6lf(%9.6lf) %14.10lf  %14.10lf  %14.10lf  %14.10lf | %14.10lf %14.10lf %14.10lf "
           "%14.10lf %14.10lf\n",
           integ.isStateInterpolated() ? "INTERP" : "------",
           s.getTime(),
           integ.getAdvancedTime(),
           s.getY()[0],
           s.getY()[1],
           s.getY()[2],
           s.getY()[3],
            ...
            switch (integ.stepTo(reportNo * hReport, nextScheduledEvent)) {
                ...
                case Integrator::ReachedScheduledEvent: {
                    ...
                    sys.handleEvents(integ.updAdvancedState(),
                                     ...
                                     handleOpts,
                                     handleResults);
                    shouldTerminate = handleResults.getExitStatus() == HandleEventsResults::ShouldTerminate;
                ...
                case Integrator::EndOfSimulation: {
                    Stage lowestModified = Stage::Empty;
                    bool shouldTerminate;
                    String reason = integ.getTerminationReasonString(integ.getTerminationReason());
...

{
    ...
    do {
        ...
        Vec2 Pinv(q / r2);
        Vec2 PWinv = Vec2(square(wq[1]) * wq[0] * q[0], square(wq[0]) * wq[1] * q[1]) / (tp * wqr2);
