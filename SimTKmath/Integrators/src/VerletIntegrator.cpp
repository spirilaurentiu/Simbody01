...
// interwoven here.
bool VerletIntegratorRep::attemptDAEStep(Real t1, Vector& yErrEst, int& errOrder, int& numIterations) {
    ...
    try {
        ...
        const Real projectionLimit =
            std::max(2 * getConstraintToleranceInUse(), std::sqrt(getConstraintToleranceInUse()));

        bool anyChangesQ;
        if (!localProjectQAndQErrEstNoThrow(advanced, dummyErrEst, anyChangesQ, projectionLimit)) {
        ...
        for (int i = 0; !converged && i < 10; ++i) {
            ...
            // will be invalid.
            // std::cout << "VerletIntegratorRep::attemptDAEStep " << advanced.getUDot() << std::endl <<
            // std::flush;
        ...
        bool anyChangesU;
        if (!localProjectUAndUErrEstNoThrow(advanced, yErrEst, anyChangesU, projectionLimit)) {
            return false; // convergence failure for this step
        }
        ...
        uErrEst *= h;
        zErrEst *= h; // everything is 3rd order in h now
        errOrder = 3;
