...
class MyFrameController : public Visualizer::FrameController {
    ...
    virtual void generateControls(const Visualizer& viz,
        ...
        geometry.push_back(
            DecorativeLine(Vec3(0), gmag * downDir).setColor(Green).setLineThickness(4).setBodyId(0));
    }
...
class MyListener : public Visualizer::InputListener {
    ...
    virtual bool keyPressed(unsigned key, unsigned modifier) override {
        String mod;
        if (modifier & ControlIsDown) {
            mod += "CTRL ";
        }
        if (modifier & ShiftIsDown) {
            mod += "SHIFT ";
        }
        if (modifier & AltIsDown) {
            mod += "ALT ";
        }
        ...
        if (modifier & IsSpecialKey) {
            std::cout << "Listener saw special key hit: " << mod << " key=" << key
                      << " glut=" << (key & ~SpecialKeyOffset);
        } else {
            std::cout << "Listener saw ordinary key hit: " << mod << char(key) << " (" << (int)key << ")";
        }
    ...
    virtual void handleEvent(State& state, Real accuracy, bool& shouldTerminate) const override {
        while (m_silo.isAnyUserInput()) {
            ...
            while (m_silo.takeSliderMove(whichSlider, sliderValue)) {
                ...
                if (other.norm() >= SignificantReal) {
                    other *= (remaining / other.norm());
                }
                gdir[axis] = sliderValue;
                gdir[prev] = other[0];
                gdir[next] = other[1];
                if (gdir.norm() < SignificantReal) {
                    gdir[next] = 1;
                }
...
int main() {
    try {
        ...
        for (int i = 0; i < NBodies; ++i) {
            MobilizedBody::Ball pendulum(lastBody,
                                         Transform(Vec3(0)),
                                         pendulumBody[i % 2],
                                         Transform(Vec3(0, 1, 0))); // alternate solid, translucent
            lastBody = pendulum;
        }
        ...
        viz.addFrameController(new MyFrameController(matter, MobilizedBodyIndex(NBodies / 2), gravity));
