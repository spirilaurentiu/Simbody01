...
MobilizedBodyIndex SimbodyMatterSubsystemRep::adoptMobilizedBody(MobilizedBodyIndex parentIx,
                                                                 MobilizedBody& child) {
    invalidateSubsystemTopologyCache();
...
UnilateralContactIndex SimbodyMatterSubsystemRep::adoptUnilateralContact(UnilateralContact* child) {
    assert(child);
    invalidateSubsystemTopologyCache();
...
StateLimitedFrictionIndex SimbodyMatterSubsystemRep::adoptStateLimitedFriction(StateLimitedFriction* child) {
    assert(child);
    invalidateSubsystemTopologyCache();
...
void SimbodyMatterSubsystemRep::createGroundBody() {
    ...
    mobilizedBodies[GroundIndex]->updImpl().setMyMatterSubsystem(updMySimbodyMatterSubsystemHandle(),
...
Array_<MobilizedBodyIndex> SimbodyMatterSubsystemRep::getChildren(MobilizedBodyIndex body) const {
...
const MassProperties& SimbodyMatterSubsystemRep::getDefaultBodyMassProperties(MobilizedBodyIndex body) const {
    return getRigidBodyNode(body).getMassProperties_OB_B();
}

const Transform& SimbodyMatterSubsystemRep::getDefaultMobilizerFrame(MobilizedBodyIndex body) const {
    return getRigidBodyNode(body).getX_BM();
}

const Transform& SimbodyMatterSubsystemRep::getDefaultMobilizerFrameOnParent(MobilizedBodyIndex body) const {
    return getRigidBodyNode(body).getX_PF();
}

const Transform& SimbodyMatterSubsystemRep::getBodyTransform(const State& s, MobilizedBodyIndex body) const {
    return getRigidBodyNode(body).getX_GB(getTreePositionCache(s));
}

const SpatialVec& SimbodyMatterSubsystemRep::getBodyVelocity(const State& s, MobilizedBodyIndex body) const {
    return getRigidBodyNode(body).getV_GB(getTreeVelocityCache(s));
}

const SpatialVec& SimbodyMatterSubsystemRep::getBodyAcceleration(const State& s,
                                                                 MobilizedBodyIndex body) const {
    return getRigidBodyNode(body).getA_GB(getTreeAccelerationCache(s));
}

const SpatialVec& SimbodyMatterSubsystemRep::getMobilizerCoriolisAcceleration(const State& s,
                                                                              MobilizedBodyIndex body) const {
    return getRigidBodyNode(body).getMobilizerCoriolisAcceleration(getTreeVelocityCache(s));
}
const SpatialVec& SimbodyMatterSubsystemRep::getTotalCoriolisAcceleration(const State& s,
                                                                          MobilizedBodyIndex body) const {
    return getRigidBodyNode(body).getTotalCoriolisAcceleration(getTreeVelocityCache(s));
}
const SpatialVec& SimbodyMatterSubsystemRep::getGyroscopicForce(const State& s,
                                                                MobilizedBodyIndex body) const {
    return getRigidBodyNode(body).getGyroscopicForce(getTreeVelocityCache(s));
}
const SpatialVec& SimbodyMatterSubsystemRep::getTotalCentrifugalForces(const State& s,
                                                                       MobilizedBodyIndex body) const {
    return getRigidBodyNode(body).getTotalCentrifugalForces(getTreeVelocityCache(s));
}
const SpatialVec& SimbodyMatterSubsystemRep::getArticulatedBodyCentrifugalForces(const State& s,
                                                                                 MobodIndex body) const {
    return getRigidBodyNode(body).getArticulatedBodyCentrifugalForces(getArticulatedBodyVelocityCache(s));
}
...
int SimbodyMatterSubsystemRep::realizeSubsystemTopologyImpl(State& s) const {
    SimTK_STAGECHECK_EQ_ALWAYS(getStage(s), Stage::Empty, "SimbodyMatterSubsystem::realizeTopology()");
    ...
    tc.modelingVarsIndex = allocateDiscreteVariable(s, Stage::Model, new Value<SBModelVars>(mvars));

    tc.modelingCacheIndex = allocateCacheEntry(s, Stage::Model, new Value<SBModelCache>());

    SBInstanceVars iv;
    iv.allocate(topologyCache);
    setDefaultInstanceValues(mvars, iv); // sets lock-by-default, but not q or u
    tc.topoInstanceVarsIndex = allocateDiscreteVariable(s, Stage::Instance, new Value<SBInstanceVars>(iv));
    tc.instanceCacheIndex = allocateCacheEntry(s, Stage::Instance, new Value<SBInstanceCache>());

    // Allocate the rest of the cache entries now although they won't get
    // any interesting content until later.

    tc.timeCacheIndex = allocateCacheEntry(s, Stage::Time, new Value<SBTimeCache>());
    ...
    tc.treePositionCacheIndex = s.allocateCacheEntryWithPrerequisites(getMySubsystemIndex(),
                                                                      Stage::Instance,
                                                                      Stage::Position,
                                                                      true /*q*/,
                                                                      false /*u*/,
    ...
    tc.constrainedPositionCacheIndex =
        allocateLazyCacheEntry(s, Stage::Time, new Value<SBConstrainedPositionCache>());
    ...
    tc.constrainedVelocityCacheIndex =
        allocateLazyCacheEntry(s, Stage::Position, new Value<SBConstrainedVelocityCache>());
    ...
    tc.articulatedBodyVelocityCacheIndex = s.allocateCacheEntryWithPrerequisites(
        ...
        false /*z*/,
        {} /*dv*/,
        {CacheEntryKey(getMySubsystemIndex(), tc.treeVelocityCacheIndex),
         CacheEntryKey(getMySubsystemIndex(), tc.articulatedBodyInertiaCacheIndex)},
        new Value<SBArticulatedBodyVelocityCache>());

    tc.dynamicsCacheIndex = allocateCacheEntry(s, Stage::Dynamics, new Value<SBDynamicsCache>());
    ...
    tc.treeAccelerationCacheIndex =
        allocateLazyCacheEntry(s, Stage::Dynamics, new Value<SBTreeAccelerationCache>());
    ...
    tc.constrainedAccelerationCacheIndex =
        allocateLazyCacheEntry(s, Stage::Dynamics, new Value<SBConstrainedAccelerationCache>());

    tc.valid = true;

    // Allocate a cache entry for the topologyCache, and save a copy there.
    mThis->topologyCacheIndex = allocateCacheEntry(s, Stage::Topology, new Value<SBTopologyCache>(tc));
    return 0;
}
...
int SimbodyMatterSubsystemRep::realizeSubsystemModelImpl(State& s) const {
    SimTK_STAGECHECK_EQ_ALWAYS(getStage(s), Stage::Topology, "SimbodyMatterSubsystem::realizeModel()");
    ...
    for (int i = 0; i < (int)rbNodeLevels.size(); ++i) {
        for (int j = 0; j < (int)rbNodeLevels[i].size(); ++j) {
            ...
            SBModelPerMobodInfo& mbInfo = mc.updMobodModelInfo(node.getNodeNum());
            ...
            if ((mbInfo.nQPoolInUse = node.calcQPoolSize(mv)) != 0) {
                mbInfo.startInQPool = MobodQPoolIndex(mc.totalNQPoolInUse);
                mc.totalNQPoolInUse += mbInfo.nQPoolInUse;
            } else {
                mbInfo.startInQPool.invalidate();
            }
        }
    }
    ...
    mc.timeVarsIndex = allocateDiscreteVariable(s, Stage::Time, new Value<SBTimeVars>());
    ...
    mc.dynamicsVarsIndex = allocateDiscreteVariable(s, Stage::Dynamics, new Value<SBDynamicsVars>(dvars));
...
int SimbodyMatterSubsystemRep::realizeSubsystemInstanceImpl(const State& s) const {
    SimTK_STAGECHECK_GE_ALWAYS(getStage(s),
                               Stage(Stage::Instance).prev(),
    ...
    for (MobilizedBodyIndex mbx(0); mbx < mobilizedBodies.size(); ++mbx) {
        ...
        switch (instanceInfo.qMethod) {
            ...
            default:
                SimTK_ASSERT1_ALWAYS(!"qMethod",
                                     ...
                                     "Unrecognized qMethod %d.",
                                     (int)instanceInfo.qMethod);
        }
        ...
        switch (instanceInfo.uMethod) {
            ...
            default:
                SimTK_ASSERT1_ALWAYS(!"uMethod",
                                     "SimbodyMatterSubsystemRep::realizeSubsystemInstanceImpl(): "
                                     "Unrecognized uMethod %d.",
                                     (int)instanceInfo.uMethod);
        }
        ...
        switch (instanceInfo.udotMethod) {
            ...
            default:
                SimTK_ASSERT1_ALWAYS(!"udotMethod",
                                     "SimbodyMatterSubsystemRep::realizeSubsystemInstanceImpl(): "
                                     "Unrecognized udotMethod %d.",
                                     (int)instanceInfo.udotMethod);
        }
        ...
        if (instanceInfo.udotMethod != Motion::Free) {
            instanceInfo.firstPresForce = PresForcePoolIndex(ic.presForce.size());
            for (int i = 0; i < nu; ++i) {
...
//==============================================================================
void SimbodyMatterSubsystemRep::realizePositionKinematics(const State& state) const {
    ...
    SimTK_STAGECHECK_GE_ALWAYS(getStage(state),
                               Stage::Instance,
    ...
    for (ConstraintIndex cx(0); cx < constraints.size(); ++cx) {
        getConstraint(cx).getImpl().calcConstrainedBodyTransformInAncestor(iv, tpc);
    }
...
bool SimbodyMatterSubsystemRep::isPositionKinematicsRealized(const State& state) const {
    const CacheEntryIndex tpcx = topologyCache.treePositionCacheIndex;
    return isCacheValueRealized(state, tpcx);
}

void SimbodyMatterSubsystemRep::invalidatePositionKinematics(const State& state) const {
...
bool SimbodyMatterSubsystemRep::isCompositeBodyInertiasRealized(const State& state) const {
    const CacheEntryIndex cbx = topologyCache.compositeBodyInertiaCacheIndex;
    return isCacheValueRealized(state, cbx);
}

void SimbodyMatterSubsystemRep::invalidateCompositeBodyInertias(const State& state) const {
...
//  - they were last realized since any change to Stage::Instance,
//    PositionKinematics, and generalized coordinates q.
bool SimbodyMatterSubsystemRep::isArticulatedBodyInertiasRealized(const State& state) const {
    const CacheEntryIndex abx = topologyCache.articulatedBodyInertiaCacheIndex;
    return isCacheValueRealized(state, abx);
}

void SimbodyMatterSubsystemRep::invalidateArticulatedBodyInertias(const State& state) const {
...
int SimbodyMatterSubsystemRep::realizeSubsystemVelocityImpl(const State& s) const {
    ...
    for (ConstraintIndex cx(0); cx < constraints.size(); ++cx) {
        ...
        if (mNonholo) {
            Real* verrp = &uErr[ic.totalNHolonomicConstraintEquationsInUse + nonholoseg.offset];
...
//==============================================================================
void SimbodyMatterSubsystemRep::realizeVelocityKinematics(const State& state) const {
    ...
    for (ConstraintIndex cx(0); cx < constraints.size(); ++cx) {
        getConstraint(cx).getImpl().calcConstrainedBodyVelocityInAncestor(iv, tpc, tvc);
    }
...
bool SimbodyMatterSubsystemRep::isVelocityKinematicsRealized(const State& state) const {
    const CacheEntryIndex velx = topologyCache.treeVelocityCacheIndex;
    return isCacheValueRealized(state, velx);
}

void SimbodyMatterSubsystemRep::invalidateVelocityKinematics(const State& state) const {
...
//==============================================================================
void SimbodyMatterSubsystemRep::realizeArticulatedBodyVelocity(const State& state) const {
    const CacheEntryIndex abvx = topologyCache.articulatedBodyVelocityCacheIndex;
    ...
    const SBArticulatedBodyInertiaCache& abc = getArticulatedBodyInertiaCache(state);
    SBArticulatedBodyVelocityCache& abvc = updArticulatedBodyVelocityCache(state);
    ...
    for (int i = 1; i < (int)rbNodeLevels.size(); i++) {
        for (int j = 0; j < (int)rbNodeLevels[i].size(); j++) {
            rbNodeLevels[i][j]->realizeArticulatedBodyVelocityCache(tpc, tvc, abc, abvc);
        }
    }
...
bool SimbodyMatterSubsystemRep::isArticulatedBodyVelocityRealized(const State& state) const {
    const CacheEntryIndex abvx = topologyCache.articulatedBodyVelocityCacheIndex;
    return isCacheValueRealized(state, abvx);
}

void SimbodyMatterSubsystemRep::invalidateArticulatedBodyVelocity(const State& state) const {
...
//==============================================================================
int SimbodyMatterSubsystemRep::calcDecorativeGeometryAndAppendImpl(const State& s,
                                                                   Stage stage,
...
void SimbodyMatterSubsystemRep::setDefaultInstanceValues(const SBModelVars& mv, SBInstanceVars& iv) const {
    // Node/joint-level defaults
    for (int i = 0; i < (int)rbNodeLevels.size(); i++) {
        for (int j = 0; j < (int)rbNodeLevels[i].size(); j++) {
    ...
    for (MobilizedBodyIndex mbx(0); mbx < getNumBodies(); ++mbx) {
        ...
        iv.prescribedMotionIsDisabled[mbx] =
            mobod.hasMotion() ? mobod.getMotion().isDisabledByDefault() : false;
    }
...
void SimbodyMatterSubsystemRep::setDefaultTimeValues(const SBModelVars& mv, SBTimeVars& timeVars) const {
...
void SimbodyMatterSubsystemRep::setConstraintIsDisabled(State& s,
...
QuaternionPoolIndex SimbodyMatterSubsystemRep::getQuaternionPoolIndex(const State& s,
                                                                      MobilizedBodyIndex body) const {
    const SBModelCache& mc = getModelCache(s); // must be >=Model stage
...
const Array_<QIndex>& SimbodyMatterSubsystemRep::getFreeQIndex(const State& state) const {
    const SBInstanceCache& ic = getInstanceCache(state);
    return ic.freeQ;
}

const Array_<QIndex>& SimbodyMatterSubsystemRep::getPresQIndex(const State& state) const {
    const SBInstanceCache& ic = getInstanceCache(state);
    return ic.presQ;
}
const Array_<QIndex>& SimbodyMatterSubsystemRep::getZeroQIndex(const State& state) const {
    const SBInstanceCache& ic = getInstanceCache(state);
    return ic.zeroQ;
}

const Array_<UIndex>& SimbodyMatterSubsystemRep::getFreeUIndex(const State& state) const {
    const SBInstanceCache& ic = getInstanceCache(state);
    return ic.freeU;
}

const Array_<UIndex>& SimbodyMatterSubsystemRep::getPresUIndex(const State& state) const {
    const SBInstanceCache& ic = getInstanceCache(state);
    return ic.presU;
}
const Array_<UIndex>& SimbodyMatterSubsystemRep::getZeroUIndex(const State& state) const {
    const SBInstanceCache& ic = getInstanceCache(state);
    return ic.zeroU;
}

const Array_<UIndex>& SimbodyMatterSubsystemRep::getFreeUDotIndex(const State& state) const {
    const SBInstanceCache& ic = getInstanceCache(state);
    return ic.freeUDot;
}

const Array_<UIndex>& SimbodyMatterSubsystemRep::getKnownUDotIndex(const State& state) const {
    const SBInstanceCache& ic = getInstanceCache(state);
    return ic.presForce;
}
...
//==============================================================================
void SimbodyMatterSubsystemRep::packFreeQ(const State& s, const Vector& allQ, Vector& packedFreeQ) const {
...
void SimbodyMatterSubsystemRep::unpackFreeQ(const State& s,
...
void SimbodyMatterSubsystemRep::zeroKnownQ(const State& s, Vector& qlike) const { // known = prescribed + zero
    const int nq = getNQ(s);
...
void SimbodyMatterSubsystemRep::packFreeU(const State& s, const Vector& allU, Vector& packedFreeU) const {
...
void SimbodyMatterSubsystemRep::unpackFreeU(const State& s,
...
void SimbodyMatterSubsystemRep::zeroKnownU(const State& s, Vector& ulike) const { // known = prescribed + zero
    const int nu = getNU(s);
...
// the combined forces.
void SimbodyMatterSubsystemRep::calcConstraintForcesFromMultipliers(const State& s,
                                                                    const Vector& lambda,
    ...
    bodyForcesInG.resize(getNumBodies());
    bodyForcesInG.setToZero();
    mobilityForces.resize(getNU(s));
    ...
    for (ConstraintIndex cx(0); cx < constraints.size(); ++cx) {
        ...
        const int mp = holoSeg.length, mv = nonholoSeg.length, ma = accOnlySeg.length;
        ...
        crep.calcConstraintForcesFromMultipliers(s, lambdap, lambdav, lambdaa, bodyF1_G, mobilityF1);
        ...
        // the Ground frame.
        for (ConstrainedBodyIndex cbx(0); cbx < ncb; ++cbx) {
            bodyForcesInG[crep.getMobilizedBodyIndexOfConstrainedBody(cbx)] += bodyF1_G[cbx]; // 6 flops
        }

        // Unpack constrained mobility forces and add them into global array.
        for (ConstrainedUIndex cux(0); cux < ncu; ++cux) {
            mobilityForces[cInfo.getUIndexFromConstrainedU(cux)] += mobilityF1[cux]; // 1 flop
        }
    }
}
...
// Complexity is O(m+n).
void SimbodyMatterSubsystemRep::multiplyByPVATranspose(const State& s,
                                                       bool includeP,
                                                       bool includeV,
                                                       bool includeA,
                                                       const Vector& lambda,
    ...
    // Global problem dimensions.
    const int mHolo = includeP ? ic.totalNHolonomicConstraintEquationsInUse : 0;
    const int mNonholo = includeV ? ic.totalNNonholonomicConstraintEquationsInUse : 0;
    const int mAccOnly = includeA ? ic.totalNAccelerationOnlyConstraintEquationsInUse : 0;
    const int m = mHolo + mNonholo + mAccOnly;
    ...
    ArrayViewConst_<Real> allLambdav(first + mHolo, first + mHolo + mNonholo);
    ArrayViewConst_<Real> allLambdaa(first + mHolo + mNonholo, first + mHolo + mNonholo + mAccOnly);
    ...
    for (ConstraintIndex cx(0); cx < constraints.size(); ++cx) {
        ...
        if (mp) {
            ...
            onefq.resize(ncq);
            onefq.fill(Real(0));
            ArrayViewConst_<Real> lambdap(&allLambdap[holoSeg.offset], &allLambdap[holoSeg.offset] + mp);
...
void SimbodyMatterSubsystemRep::calcPVATranspose(const State& s,
                                                 bool includeP,
                                                 bool includeV,
                                                 bool includeA,
                                                 Matrix& PVAt) const {
    ...
    for (int j = 0; j < m; ++j) {
        ...
        if (isContiguous) {
            multiplyByPVATranspose(s, includeP, includeV, includeA, lambda, PVAt(j));
        } else {
            multiplyByPVATranspose(s, includeP, includeV, includeA, lambda, contig_col);
            PVAt(j) = contig_col;
        }
        lambda[j] = 0;
    }
...
void SimbodyMatterSubsystemRep::multiplyByPqTranspose(const State& state,
...
void SimbodyMatterSubsystemRep::calcPqTranspose(const State& s, Matrix& Pqt) const {
    const SBInstanceCache& ic = getInstanceCache(s);
...
void SimbodyMatterSubsystemRep::calcWeightedPqrTranspose(const State& s,
...
void SimbodyMatterSubsystemRep::calcWeightedPVrTranspose(const State& s,
...
// Complexity is O(m).
void SimbodyMatterSubsystemRep::calcBiasForMultiplyByPVA(const State& s,
                                                         bool includeP,
                                                         bool includeV,
                                                         bool includeA,
                                                         Vector& bias) const {
    ...
    for (ConstraintIndex cx(0); cx < constraints.size(); ++cx) {
        ...
        if (mp) { // holonomic -- use velocity equations
            ...
            if (zeroV_AB.size() < ncb) {
                zeroV_AB.resize(ncb, SpatialVec(Vec3(0)));
            }
            if (zeroQDot.size() < ncq) {
                zeroQDot.resize(ncq, Real(0));
            }

            // Make subarrays; this does not require heap allocation.
            const ArrayViewConst_<SpatialVec, ConstrainedBodyIndex> V0_AB =
                zeroV_AB(ConstrainedBodyIndex(0), ncb);
            const ArrayViewConst_<Real, ConstrainedQIndex> qdot0 = zeroQDot(ConstrainedQIndex(0), ncq);
        ...
        if (zeroUDot.size() < ncu) {
            zeroUDot.resize(ncu, Real(0));
        }
        // Make a subarray of the right size.
        const ArrayViewConst_<Real, ConstrainedUIndex> udot0 = zeroUDot(ConstrainedUIndex(0), ncu);
...
// The output vector must use contiguous storage.
// Complexity is O(m).
void SimbodyMatterSubsystemRep::calcBiasForAccelerationConstraints(const State& s,
                                                                   bool includeP,
                                                                   bool includeV,
                                                                   bool includeA,
                                                                   Vector& bias) const {
    ...
    // are AC_GB, the coriolis accelerations in Ground. The constraint methods
    // want those relative to their Ancestor frames, which might not be Ground.
    const Array_<SpatialVec, MobilizedBodyIndex>& allAC_GB =
        getTreeVelocityCache(s).totalCoriolisAcceleration;
    ...
    for (ConstraintIndex cx(0); cx < constraints.size(); ++cx) {
        ...
        if (mp) { // holonomic -- use velocity equations
            const int ncq = cInfo.getNumConstrainedQ();
            // Make sure we have enough zeroes.
            if (zeroQDotDot.size() < ncq) {
                zeroQDotDot.resize(ncq, Real(0));
            }

            // Make subarray; this does not require heap allocation.
            const ArrayViewConst_<Real, ConstrainedQIndex> qdotdot0 = zeroQDotDot(ConstrainedQIndex(0), ncq);
...
// Complexity is O(mp + n).
void SimbodyMatterSubsystemRep::multiplyByPq(const State& s,
                                             const Vector& bias_p,
                                             const Vector& qlike,
    ...
    const ArrayViewConst_<SpatialVec, MobilizedBodyIndex> V_GBArray(&V_GB[0], &V_GB[0] + nb);
...
void SimbodyMatterSubsystemRep::calcPq(const State& s, Matrix& Pq) const {
    const SBInstanceCache& ic = getInstanceCache(s);
...
// All of the Vector arguments must use contiguous storage.
// Complexity is O(m+n).
void SimbodyMatterSubsystemRep::multiplyByPVA(const State& s,
                                              bool includeP,
                                              bool includeV,
                                              bool includeA,
                                              const Vector& bias,
    ...
    const ArrayViewConst_<SpatialVec, MobilizedBodyIndex> allV_GB(&Julike[0], &Julike[0] + nb);
    ...
    if (mNonholo || mAccOnly) {
        allA_GB.resize(nb);
        const Array_<SpatialVec>& allAC_GB = getTreeVelocityCache(s).totalCoriolisAcceleration;
...
void SimbodyMatterSubsystemRep::calcPVA(const State& s,
                                        bool includeP,
                                        bool includeV,
                                        bool includeA,
                                        Matrix& PVA) const {
    ...
    for (int j = 0; j < nu; ++j) {
        ...
        if (isContiguous) {
            multiplyByPVA(s, includeP, includeV, includeA, bias, ulike, PVA(j));
        } else {
            multiplyByPVA(s, includeP, includeV, includeA, bias, ulike, contig_col);
            PVA(j) = contig_col;
        }
        ulike[j] = 0;
    }
...
void SimbodyMatterSubsystemRep::calcGMInvGt(const State& s, Matrix& GMInvGt) const {
    const SBInstanceCache& ic = getInstanceCache(s);
...
void SimbodyMatterSubsystemRep::solveForConstraintImpulses(const State& state,
                                                           const Vector& deltaV,
                                                           Vector& impulse) const {
    Matrix GMInvGt;
    calcGMInvGt(state, GMInvGt);
    // MUST DUPLICATE SIMBODY'S METHOD HERE:
    const Real conditioningTol = GMInvGt.nrow() * SqrtEps * std::sqrt(SqrtEps); // Eps^(3/4)
...
void SimbodyMatterSubsystemRep::calcBodyAccelerationFromUDot(const State& s,
    ...
    for (int i = 0; i < (int)rbNodeLevels.size(); i++) {
        for (int j = 0; j < (int)rbNodeLevels[i].size(); j++) {
            const RigidBodyNode& node = *rbNodeLevels[i][j];
            node.calcBodyAccelerationsFromUdotOutward(tpc, tvc, knownUdotPtr, aPtr);
        }
    }
}
...
void SimbodyMatterSubsystemRep::calcConstraintAccelerationErrors(const State& s,
    ...
    // Overlay Arrays on the Vectors' data so that we can manipulate small
    // chunks of them repeatedly with no heap activity or virtual method calls.
    const ArrayViewConst_<SpatialVec, MobilizedBodyIndex> allA_GB(&A_GB[0], &A_GB[0] + nb);
...
bool SimbodyMatterSubsystemRep::prescribeQ(State& s) const {
    ...
    if (npq == 0 && nzq == 0) {
        return false; // don't invalidate positions
    }
...
bool SimbodyMatterSubsystemRep::prescribeU(State& s) const {
    ...
    if (npu == 0 && nzu == 0) {
        return false; // don't invalidate velocities
    }
...
static Real calcQErrestWeightedNormU(const SimbodyMatterSubsystemRep& matter,
                                     const State& s,
                                     const Vector& qErrest,
                                     const Vector& uWeights) {
...
void SimbodyMatterSubsystemRep::enforcePositionConstraints(State& s,
    ...
    const Real consAccuracyToTryFor = std::max(Real(0.1) * consAccuracy, SignificantReal);
    ...
    if (normAchievedTRMS > consAccuracyToTryFor) {
        ...
        do {
            ...
            if (localOnly && nItsUsed >= 2 && normAchievedTRMS > prevNormAchievedTRMS) {
        ...
        } while (normAchievedTRMS > consAccuracyToTryFor && nItsUsed < MaxIterations);

        // Make sure we achieved at least the required constraint accuracy.
        if (normAchievedTRMS > consAccuracy) {
            updQ(s) = saveQ; // revert
            realizeSubsystemPosition(s);
            SimTK_THROW1(Exception::NewtonRaphsonFailure, "Failed to converge in position projection");
        }
    ...
    if (mQuats) {
        ...
        for (int i = 0; i < (int)rbNodeLevels.size(); i++) {
            for (int j = 0; j < (int)rbNodeLevels[i].size(); j++) {
                const RigidBodyNode& node = *rbNodeLevels[i][j];
                const SBInstancePerMobodInfo& mobodInfo = ic.mobodInstanceInfo[node.getNodeNum()];
...
int SimbodyMatterSubsystemRep::projectQ(State& s,
                                        Vector& qErrest, // q error estimate or empty
                                        const ProjectOptions& opts,
                                        ProjectResults& results) const {
    SimTK_STAGECHECK_GE(getStage(s), Stage::Position, "SimbodyMatterSubsystemRep::projectQ()");
    ...
    const Real perrNormOnEntry =
        useNormInf ? scaledPerrs.normInf(&worstPerr) : scaledPerrs.normRMS(&worstPerr);
    const Real quatNormOnEntry =
        useNormInf ? quatErrs.normInf(&worstQuatErr) : quatErrs.normRMS(&worstQuatErr);
    ...
    if (perrNormOnEntry == 0 || (perrNormOnEntry <= consAccuracy && !forceOneIter)) {
    ...
    const Real consAccuracyToTryFor = std::max(overshootFactor * consAccuracy, SignificantReal);
    ...
    const bool forceFullNewton = opts.isOptionSet(ProjectOptions::ForceFullNewton);
    ...
    do {
        ...
        perrNormAchieved = useNormInf ? scaledPerrs.normInf() : scaledPerrs.normRMS();
        ++nItsUsed;

        if (localOnly && nItsUsed >= 2 && perrNormAchieved > prevPerrNormAchieved) {
            // perr norm got worse; restore to end of previous iteration
            updQ(s) += dq;
            realizeSubsystemPosition(s); // pErrs changes here
    ...
    } while (perrNormAchieved > consAccuracyToTryFor && nItsUsed < MaxIterations);
    ...
    if (mQuats) {
        ...
        quatNormAchieved = useNormInf ? quatErrs.normInf(&worstQuatErr) : quatErrs.normRMS(&worstQuatErr);
...
bool SimbodyMatterSubsystemRep::normalizeQuaternions(State& s, Vector& qErrest) const {
    SBStateDigest sbs(s, *this, Stage::Model);
    const SBInstanceCache& ic = getInstanceCache(s);
...
//==============================================================================
void SimbodyMatterSubsystemRep::enforceVelocityConstraints(State& s,
                                                           Real consAccuracy,
                                                           const Vector& yWeights,
                                                           const Vector& ooTols,
                                                           Vector& yErrest,
    ...
    if (normAchievedTRMS > consAccuracyToTryFor) {
        ...
        if (normAchievedTRMS > consAccuracy) {
            updU(s) = saveU;
            realizeSubsystemVelocity(s);
            SimTK_THROW1(Exception::NewtonRaphsonFailure, "Failed to converge in velocity projection");
        }
        ...
        if (uErrest.size()) {
            ...
            if (hasPrescribedMotion) {
                ...
                multiplyByPVA(s, true, true, false, bias_pv, uErrest_0, Tpv_PV_uErrest);
...
//                                 PROJECT U
//==============================================================================
int SimbodyMatterSubsystemRep::projectU(State& s,
                                        Vector& uErrest, // u error estimate or empty
                                        const ProjectOptions& opts,
                                        ProjectResults& results) const {
    SimTK_STAGECHECK_GE(getStage(s), Stage::Velocity, "SimbodyMatterSubsystemRep::projectU()");
    ...
    const Real pverrNormOnEntry =
        useNormInf ? scaledPVerrs.normInf(&worstPVerr) : scaledPVerrs.normRMS(&worstPVerr);
    ...
    // aren't any), or (b) constraints are already satisfied and we're
    // being forced to go ahead anyway.
    if (pverrNormOnEntry == 0 || (pverrNormOnEntry <= consAccuracy && !forceOneIter)) {
        // numIterations==0.
        results.setAnyChangeMade(false);
    ...
    do {
        ...
        pverrNormAchieved = useNormInf ? scaledPVerrs.normInf() : scaledPVerrs.normRMS();
        ++nItsUsed;

        if (localOnly && nItsUsed >= 2 && pverrNormAchieved > prevPVerrNormAchieved) {
            // Velocity norm worse -- restore to end of previous iteration.
            updU(s) += du;
    ...
    } while (pverrNormAchieved > consAccuracyToTryFor && nItsUsed < MaxIterations);
...
void SimbodyMatterSubsystemRep::calcTreeForwardDynamicsOperator(
    ...
    Array_<SpatialVec, MobilizedBodyIndex>& abForcesZ = tac.z;
    Array_<SpatialVec, MobilizedBodyIndex>& abForcesZPlus = tac.zPlus;
...
void SimbodyMatterSubsystemRep::realizeTreeForwardDynamics(const State& s,
                                                           const Vector& mobilityForces,
                                                           const Vector_<Vec3>& particleForces,
                                                           const Vector_<SpatialVec>& bodyForces,
                                                           const Vector* extraMobilityForces,
    ...
    calcTreeForwardDynamicsOperator(s,
                                    mobilityForces,
                                    particleForces,
                                    bodyForces,
                                    extraMobilityForces,
...
void SimbodyMatterSubsystemRep::calcLoopForwardDynamicsOperator(const State& s,
    ...
    Vector_<SpatialVec> bodyForcesInG;
    Vector mobilityF;
    calcConstraintForcesFromMultipliers(s,
                                        multipliers,
                                        bodyForcesInG,
                                        mobilityF,
                                        cac.constrainedBodyForcesInG,
                                        cac.constraintMobilityForces);
    // Note that constraint forces have the opposite sign from applied forces
...
void SimbodyMatterSubsystemRep::realizeLoopForwardDynamics(const State& s,
    ...
    calcLoopForwardDynamicsOperator(s,
                                    mobilityForces,
                                    particleForces,
                                    bodyForces,
                                    tac,
...
//
void SimbodyMatterSubsystemRep::calcCompositeBodyInertias(
    const State& s,
...
void SimbodyMatterSubsystemRep::calcTreeAccelerations(const State& s,
    ...
    const SBArticulatedBodyInertiaCache& abc = getArticulatedBodyInertiaCache(s);
    const SBArticulatedBodyVelocityCache& abvc = getArticulatedBodyVelocityCache(s);
    ...
    const Real* mobilityForcePtr = mobilityForces.size() ? &mobilityForces[0] : nullptr;
    const SpatialVec* bodyForcePtr = bodyForces.size() ? &bodyForces[0] : nullptr;
    Real* hingeForcePtr = netHingeForces.size() ? &netHingeForces[0] : nullptr;
    ...
    for (int i = 0; i < (int)rbNodeLevels.size(); i++) {
        for (int j = 0; j < (int)rbNodeLevels[i].size(); j++) {
            const RigidBodyNode& node = *rbNodeLevels[i][j];
            node.calcUDotPass2Outward(ic, tpc, abc, tvc, dc, hingeForcePtr, aPtr, udotPtr, tauPtr);
...
void SimbodyMatterSubsystemRep::multiplyByMInv(const State& s, const Vector& f, Vector& MInvf) const {
    const SBInstanceCache& ic = getInstanceCache(s);
    const SBTreePositionCache& tpc = getTreePositionCache(s);
    ...
    for (int i = rbNodeLevels.size() - 1; i >= 0; i--) {
        for (int j = 0; j < (int)rbNodeLevels[i].size(); j++) {
            const RigidBodyNode& node = *rbNodeLevels[i][j];
            node.multiplyByMInvPass1Inward(ic, tpc, abc, fPtr, z.begin(), zPlus.begin(), eps.begin());
        }
    }
    ...
    for (int i = 0; i < (int)rbNodeLevels.size(); i++) {
        for (int j = 0; j < (int)rbNodeLevels[i].size(); j++) {
            const RigidBodyNode& node = *rbNodeLevels[i][j];
            node.multiplyByMInvPass2Outward(ic, tpc, abc, eps.cbegin(), A_GB.begin(), MInvfPtr);
        }
    }
}
//............................. CALC M INVERSE F ...............................
...
// realize articulated body inertias here if necessary.
// All vectors must use contiguous storage.
void SimbodyMatterSubsystemRep::multiplyBySqrtMInv(const State& s, const Vector& v, Vector& sqrtMinvV) const {
    ...
    for (int i = 0; i < (int)rbNodeLevels.size(); i++) {
        for (int j = 0; j < (int)rbNodeLevels[i].size(); j++) {
            const RigidBodyNode& node = *rbNodeLevels[i][j];
            node.multiplyBySqrtMInvPassOutward(ic, tpc, abc, eps.cbegin(), V_GB.begin(), sqrtMinvVPtr);
        }
    }
}
...
// All vectors must use contiguous storage.
void SimbodyMatterSubsystemRep::calcDetM(const State& s, const Vector& f, Vector& MInvf, Real* detM) const {
    ...
    *detM = 1.0;
    for (int i = 0; i < (int)rbNodeLevels.size(); i++) {
        for (int j = 0; j < (int)rbNodeLevels[i].size(); j++) {
            ...
            // std::cout << "SimbodyMatterSubsystemRep::calcFixmanTorque node[" << i << "][" << j << "]" <<
            // std::endl; // FIXTORROLL

            node.calcFixmanTorquePass2Outward(ic, tpc, abc, dc, eps.cbegin(), A_GB.begin(), MInvfPtr, detM);
        }
    }
}
//.............................    CALC FIXMAN TORQUE  ...............................
...
// All vectors must use contiguous storage.
void SimbodyMatterSubsystemRep::multiplyByM(const State& s, const Vector& a, Vector& Ma) const {
    const SBTreePositionCache& tpc = getTreePositionCache(s);
    const int nb = getNumBodies();
    const int nu = getNU(s);
...
void SimbodyMatterSubsystemRep::calcTreeResidualForces(const State& s,
    ...
    for (int i = rbNodeLevels.size() - 1; i >= 0; i--) {
        for (int j = 0; j < (int)rbNodeLevels[i].size(); j++) {
            const RigidBodyNode& node = *rbNodeLevels[i][j];
            node.calcInverseDynamicsPass2Inward(tpc,
                                                tvc,
                                                aPtr,
                                                mobilityForcePtr,
                                                bodyForcePtr,
...
void SimbodyMatterSubsystemRep::multiplyByN(const State& s,
                                            bool transpose,
                                            const Vector& in,
                                            Vector& out) const {
...
// Both vectors must use contiguous storage.
void SimbodyMatterSubsystemRep::multiplyByNDot(const State& s,
                                               bool transpose,
                                               const Vector& in,
                                               Vector& out) const {
...
// Both vectors must use contiguous storage.
void SimbodyMatterSubsystemRep::multiplyByNInv(const State& s,
                                               bool transpose,
                                               const Vector& in,
                                               Vector& out) const {
    // i.e., we must be *done* with Stage::Position
...
void SimbodyMatterSubsystemRep::calcMobilizerReactionForces(const State& s, Vector_<SpatialVec>& FM_G) const {
    const int nb = getNumBodies();
    ...
    const Array_<ArticulatedInertia, MobilizedBodyIndex>& PPlus = getArticulatedBodyInertiasPlus(s);
    const Array_<SpatialVec, MobilizedBodyIndex>& zPlus = getArticulatedBodyForcesPlus(s);
...
void SimbodyMatterSubsystemRep::calcMobilizerReactionForcesUsingFreebodyMethod(
    ...
    Vector_<SpatialVec> otherFB_G = getMultibodySystem().getRigidBodyForces(s, Stage::Dynamics);
    ...
    calcConstraintForcesFromMultipliers(s,
                                        s.getMultipliers(),
                                        constrainedBodyForces_G,
                                        constrainedMobilizerForces);
    ...
    for (int i = (int)rbNodeLevels.size() - 1; i >= 0; --i) {
        for (int j = 0; j < (int)rbNodeLevels[i].size(); ++j) {
            ...
            const SpatialVec FB_G = totalFB_G - (otherFB_G[mbx] - getGyroscopicForce(s, mbx));
            // Shift to M
            const Transform& X_GB = body.getBodyTransform(s);
            const Vec3& p_BM = body.getOutboardFrame(s).p();
...
Vector SimbodyMatterSubsystemRep::calcMotionErrors(const State& s, const Stage& stage) const {
    const SBInstanceCache& ic = getInstanceCache(s);
    Vector errs;
    ...
    SimTK_APIARGCHECK1_ALWAYS(Stage::Position <= stage && stage <= Stage::Acceleration,
...
//==============================================================================
void SimbodyMatterSubsystemRep::findMotionForces(const State& s, Vector& mobilityForces) const {
    const SBInstanceCache& ic = getInstanceCache(s);
    const int nu = getNU(s);
...
Real SimbodyMatterSubsystemRep::calcMotionPower(const State& s) const {
    const SBInstanceCache& ic = getInstanceCache(s);
    const int nu_p = ic.getTotalNumPresForces(); // num prescribed udots
    if (nu_p == 0) {
        return 0;
...
//==============================================================================
void SimbodyMatterSubsystemRep::findConstraintForces(const State& s,
                                                     Vector_<SpatialVec>& bodyForcesInG,
...
Real SimbodyMatterSubsystemRep::calcConstraintPower(const State& s) const {
    Real power = 0;
...
void SimbodyMatterSubsystemRep::calcMobilizerQDotFromU(const State& s,
...
// this mobilizer.
void SimbodyMatterSubsystemRep::calcMobilizerQDotDotFromUDot(const State& s,
                                                             MobilizedBodyIndex mb,
                                                             int nu,
                                                             const Real* udot,
                                                             int nq,
...
// Returns X_FM(q).
Transform SimbodyMatterSubsystemRep::calcMobilizerTransformFromQ(const State& s,
                                                                 MobilizedBodyIndex mb,
                                                                 int nq,
                                                                 const Real* q) const {
...
SpatialVec SimbodyMatterSubsystemRep::calcMobilizerVelocityFromU(const State& s,
                                                                 MobilizedBodyIndex mb,
                                                                 int nu,
                                                                 const Real* u) const {
    const SBStateDigest sbState(s, *this, Stage::Position);
...
SpatialVec SimbodyMatterSubsystemRep::calcMobilizerAccelerationFromUDot(const State& s,
                                                                        MobilizedBodyIndex mb,
                                                                        int nu,
...
Transform SimbodyMatterSubsystemRep::calcParentToChildTransformFromQ(const State& s,
                                                                     MobilizedBodyIndex mb,
                                                                     int nq,
                                                                     const Real* q) const {
...
SpatialVec SimbodyMatterSubsystemRep::calcParentToChildVelocityFromU(const State& s,
                                                                     MobilizedBodyIndex mb,
                                                                     int nu,
                                                                     const Real* u) const {
    assert(!"not implemented yet");
...
SpatialVec SimbodyMatterSubsystemRep::calcParentToChildAccelerationFromUDot(const State& s,
                                                                            MobilizedBodyIndex mb,
                                                                            int nu,
                                                                            const Real* udot) const {
    assert(!"not implemented yet");
...
void SimbodyMatterSubsystemRep::multiplyBySystemJacobianTranspose(const State& s,
    ...
    Vector_<SpatialVec> zTemp(getNumBodies());
    zTemp.setToZero();
...
void SimbodyMatterSubsystemRep::calcTreeEquivalentMobilityForces(const State& s,
    ...
    for (int i = rbNodeLevels.size() - 1; i > 0; i--) {
        for (int j = 0; j < (int)rbNodeLevels[i].size(); j++) {
            const RigidBodyNode& node = *rbNodeLevels[i][j];
            node.calcEquivalentJointForces(tpc, tvc, bodyForcePtr, zPtr, mobilityForcePtr);
        }
    }
}
//.................... CALC TREE EQUIVALENT MOBILITY FORCES ....................
...
std::ostream& operator<<(std::ostream& o, const SimbodyMatterSubsystemRep& tree) {
    ...
    for (MobilizedBodyIndex i(0); i < tree.getNumBodies(); ++i) {
        o << i << "->" << tree.nodeNum2NodeMap[i].level << "," << tree.nodeNum2NodeMap[i].offset << ";";
        const RigidBodyNode& n = tree.getRigidBodyNode(i);
        o << n.getNodeNum() << "," << n.getLevel() << "(u" << n.getUIndex() << ":" << n.getDOF() << ",q"
          << n.getQIndex() << ":" << n.getMaxNQ() << ")" << std::endl;
    }
